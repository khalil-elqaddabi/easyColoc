<x-app-layout title="Edit {{ $category->name }} - {{ $category->colocation->name }}">
    <div class="flex">
        {{-- Sidebar --}}
        <x-nav></x-nav>
                ...
                <div class="bg-white dark:bg-gray-800 shadow-2xl rounded-3xl border border-gray-100 dark:border-gray-700 overflow-hidden">
                    <div class="px-8 py-10 border-b border-gray-100 dark:border-gray-700 bg-gradient-to-r from-indigo-50 to-purple-50 dark:from-indigo-900/20 dark:to-purple-900/20">
                        <div class="flex items-start gap-4">
                            ...
                            <div>
                                <h1 class="text-3xl font-bold text-gray-900 dark:text-gray-100 mb-2">Edit Category</h1>
                                <p class="text-lg text-gray-600 dark:text-gray-300">
                                    Update "{{ $category->name }}" in {{ $category->colocation->name }}
                                </p>
                            </div>
                        </div>
                    </div>

                    <form method="POST" action="{{ route('colocations.categories.update', [$category->colocation, $category]) }}" class="p-8">
                        @csrf @method('PUT')
                        
                        <div class="space-y-8">
                            <div>
                                <label for="name" class="block text-sm font-semibold text-gray-700 dark:text-gray-200 mb-3">
                                    Category Name
                                </label>
                                <div class="relative">
                                    <input type="text" 
